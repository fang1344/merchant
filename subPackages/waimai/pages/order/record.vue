		...
		<view class="list-scroll-content" >
			...
			<view v-for="(item,index) in orderList" :key="index" class="order-item">
				...
				<view class="price-box">
					共
					<text class="num">{{item.goods_number}}</text>
					件商品 实付款
...
<script>
	import {getRestaurantOrderList} from '@/src/utils/api'
	...
	export default {
		...
		methods: {
			async downCallback(mescroll){
				let res = await this.getData(1,mescroll);
			},
			async upCallback(mescroll){
				let res = await this.getData(mescroll.num,mescroll)
			},
			//获取订单列表
			async getData(page,mescroll){
				let res = await getRestaurantOrderList({order_state: this.order_state,today: this.today,page: page});
				let orderList = res.data.data.map(item=>{
					//添加不同状态下订单的表现形式
					item = Object.assign(item, this.orderStateExp(item.order_state));
					//演示数据所以自己进行状态筛选
					return item;
				});
				if(page==1){
			...
			toOrderDetail(item){
				uni.navigateTo({
					url: '../../subPackages/waimai/pages/orderDetail/index?order_code='+item.order_code,
				});
			},
			//订单状态文字和颜色
			orderStateExp(state){
